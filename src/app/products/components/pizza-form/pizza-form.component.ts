...
export class PizzaFormComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (this.pizza && this.pizza.id) {
      this.exists = true;
      this.form.patchValue(this.pizza as Partial<{ name: string | null; toppings: null; }>);
    }
  
    if(this.form.get('toppings') === null ||this.form.get('toppings') !instanceof FormArray) {
      return;
    }

    (this.form
      .get('toppings') as FormControl)
      .valueChanges.pipe(
      ...
      .subscribe(value => {
        this.selected.emit(value as number[])
      });
  }
